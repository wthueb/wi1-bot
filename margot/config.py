...
if dir := os.getenv("XDG_CONFIG_HOME"):
    if os.path.isfile(os.path.join(dir, "margot", "config.yaml")):
        _config_path = os.path.join(dir, "margot", "config.yaml")

if home := os.getenv("HOME"):
    if os.path.isfile(os.path.join(home, ".config", "margot", "config.yaml")):
        _config_path = os.path.join(home, ".config", "margot", "config.yaml")

if _config_path is None:
    raise FileNotFoundError(
        "could not find ./config.yaml, $XDG_CONFIG_HOME/margot/config.yaml or"
        " $HOME/.config/margot/config.yaml"
    )
